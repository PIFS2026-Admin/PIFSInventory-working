...
export default function JobIntelligencePage() {
  ...
  const loadIntelligence = useCallback(async () => {
    ...
    try {
      ...
      const request = await fetch("/api/crm/job-intelligence", { headers: { Authorization: `Bearer ${token}` } });
  ...
  async function runAction(payload: Record<string, unknown>) {
    ...
    try {
      ...
      const request = await fetch("/api/crm/job-intelligence", {
        method: "PATCH",
    ...
    <main className={styles.page}>
      <header className={styles.header}>
        <div className={styles.titleBlock}>
          ...
          <div><span>CRM</span><h1>Job Intelligence</h1></div>
        </div>
