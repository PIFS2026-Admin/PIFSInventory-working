...
export default function EquipmentReadinessPage() {
  ...
  return <main className={styles.page}>
    <header className={`${styles.header} titan-page-header`}><div className={styles.title}><Image src="/titan_logo.jpg" alt="TITAN" width={64} height={42} priority /><div><span>DTI / OMS-103 / OMS-108</span><h1>Equipment Readiness</h1></div></div><div className={styles.headerActions}><button type="button" onClick={() => goBackOrFallback("/dti?view=jobs")}>Back</button>{data?.job ? <Link href={`/dti/pre-job/${encodeURIComponent(data.job.id)}`}>Pre-Job Checklist</Link> : null}<DtiProcedureMenu procedures={[{ documentNumber: "OMS-103", label: "Equipment Setup Guide" }, { documentNumber: "OMS-108", label: "EMI and UT Calibration" }, { documentNumber: "PFIS-IOM-001", label: "Inspection Operations Manual" }]} /><button type="button" onClick={() => void load()}>Refresh</button></div></header>
