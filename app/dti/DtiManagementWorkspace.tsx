...
const ratingLabels: Record<Exclude<Rating, "">, string> = { C: "C (3)", NI: "NI (2)", NC: "NC (1)", NA: "N/A (0)" };
...
export default function DtiManagementWorkspace() {
  ...
  const scorePreview = useMemo(() => {
    const rated = checklist.filter((item) => Boolean(ratings[item.item_code]));
    const points = rated.reduce((sum, item) => sum + (ratings[item.item_code] === "C" ? 3 : ratings[item.item_code] === "NI" ? 2 : ratings[item.item_code] === "NC" ? 1 : 0), 0);
  ...
  return <main className={styles.page}>
    ...
    {!message && tab === "jobs" ? <section className={styles.listSection}><div className={styles.resultCount}>{visibleJobs.length} connected DTI jobs</div><div className={styles.jobList}>{visibleJobs.map((job) => { const jobAudits = audits.filter((audit) => audit.job_id === job.id).length; const openFindings = findings.filter((finding) => finding.job_id === job.id && finding.finding_status !== "Closed").length; return <article key={job.id}><div><span>{job.job_number} / {job.lifecycle_status}</span><strong>{job.title}</strong><small>{job.customer_name || "No customer"} / {job.rig_name || "No rig"} / {formatDate(job.scheduled_start)}</small></div><dl><div><dt>Audits</dt><dd>{jobAudits}</dd></div><div><dt>Open Findings</dt><dd>{openFindings}</dd></div></dl><div className={styles.itemActions}><Link href={`/crm/jobs/${encodeURIComponent(job.id)}`}>Open Job</Link><button className={styles.primaryButton} type="button" onClick={() => startAudit(job.id)}>New Audit</button></div></article>; })}</div>{!visibleJobs.length ? <div className={styles.empty}>No connected DTI jobs match this search.</div> : null}</section> : null}
