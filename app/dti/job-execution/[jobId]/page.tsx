...
import DtiConnectionRefacingPanel, { type ConnectionRefacing } from "./DtiConnectionRefacingPanel";
...
type ApiResponse = { ok?: boolean; job?: Job; phases?: Phase[]; runs?: Run[]; run?: Run | null; racks?: Rack[]; calibrations?: Calibration[]; assets?: Asset[]; escalations?: Escalation[]; documents?: JobDocument[]; defectDecisions?: DefectDecision[]; tubularSpecs?: TubularSpec[]; dimensionalReadings?: DimensionalReading[]; connectionRefacing?: ConnectionRefacing[]; jointInspections?: JointInspection[]; summary?: Summary; error?: string };
...
export default function DtiJobExecutionPage() {
  ...
  return <main className={styles.page}>
    ...
    {run ? <><DtiJointInspectionPanel jobId={jobId} runId={run.id} racks={data?.racks ?? []} specs={data?.tubularSpecs ?? []} decisions={data?.defectDecisions ?? []} records={data?.jointInspections ?? []} onSaved={() => void load(run.id)} /><DtiDimensionalPanel jobId={jobId} runId={run.id} racks={data?.racks ?? []} specs={data?.tubularSpecs ?? []} readings={data?.dimensionalReadings ?? []} onSaved={() => void load(run.id)} /><DtiConnectionRefacingPanel jobId={jobId} runId={run.id} racks={data?.racks ?? []} specs={data?.tubularSpecs ?? []} documents={data?.documents ?? []} records={data?.connectionRefacing ?? []} onSaved={() => void load(run.id)} /><DtiDefectDecisionPanel jobId={jobId} runId={run.id} racks={data?.racks ?? []} decisions={data?.defectDecisions ?? []} onSaved={() => void load(run.id)} /><DtiEscalationPanel jobId={jobId} runId={run.id} racks={data?.racks ?? []} escalations={data?.escalations ?? []} documents={data?.documents ?? []} onSaved={() => void load(run.id)} /></> : null}
  </main>;
}
