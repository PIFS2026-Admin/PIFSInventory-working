...
type ApiResponse = { ok?: boolean; job?: Job; phases?: Phase[]; runs?: Run[]; run?: Run | null; racks?: Rack[]; calibrations?: Calibration[]; assets?: Asset[]; summary?: Summary; error?: string };
...
export default function DtiJobExecutionPage() {
  ...
  return <main className={styles.page}>
    <header className={`${styles.header} titan-page-header`}><div className={styles.title}><Image src="/titan_logo.jpg" alt="TITAN" width={64} height={42} priority /><div><span>DTI / OMS-104 / OMS-108</span><h1>Job Execution</h1></div></div><div className={styles.headerActions}><button type="button" onClick={() => goBackOrFallback("/dti?view=jobs")}>Back</button>{data?.job ? <Link href={`/dti/pre-job/${encodeURIComponent(data.job.id)}`}>Pre-Job</Link> : null}<Link href="/dti-summary">Daily Summary</Link><DtiProcedureMenu procedures={[{ documentNumber: "OMS-104", label: "Per-Rack Workflow" }, { documentNumber: "OMS-108", label: "EMI and UT Calibration" }, { documentNumber: "OMS-105", label: "Defect Decisions" }, { documentNumber: "OMS-106", label: "Dimensional Reference" }, { documentNumber: "OMS-107", label: "Connection and Refacing" }]} /><button type="button" onClick={() => void load(run?.id)}>Refresh</button></div></header>
