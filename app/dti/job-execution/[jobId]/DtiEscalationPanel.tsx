  ...
  return <section className={styles.escalations}>
    ...
    {open ? <div className={styles.escalationForm}>
      ...
      <div className={styles.formGrid}>
        ...
        {form.customerDecision === "Accept with Deviation" ? <label className={styles.wide}><span>Written Confirmation Attachment</span><select value={form.confirmationDocumentId} onChange={(e) => update({ confirmationDocumentId: e.target.value })}><option value="">Select attached job document</option>{documents.map((document) => <option key={document.id} value={document.id}>{document.document_type || "Document"} / {document.display_name}</option>)}</select><small>Attach the written approval to the connected job before accepting a deviation.</small></label> : null}
