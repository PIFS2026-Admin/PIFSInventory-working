...
import { dtiComponentLabel, dtiComponentTypes, dtiInspectionFields, type DtiComponentType } from "../../../lib/dtiInspectionReport";
...
export default function AcceptanceCriteriaPage() {
  ...
  const visibleSets = useMemo(() => { const query = search.trim().toLowerCase(); return sets.filter((item) => !query || [item.name, item.standard_type, item.component_type, dtiComponentLabel(item.component_type), item.customer_name].some((value) => String(value ?? "").toLowerCase().includes(query))); }, [search, sets]);
