...
type RuleForm = { ruleId: string; ruleName: string; fieldKey: string; inspectionArea: string; comparison: string; minimumValue: string; maximumValue: string; expectedValue: string; resultClassification: string; reason: string; displayOrder: string; isActive: boolean };
...
const emptyRule: RuleForm = { ruleId: "", ruleName: "", fieldKey: "", inspectionArea: "Joint", comparison: "Minimum", minimumValue: "", maximumValue: "", expectedValue: "", resultClassification: "DBR", reason: "", displayOrder: "1", isActive: true };
...
function criteriaValue(rule: Rule) {
  if (rule.comparison === "Minimum") return `>= ${rule.minimum_value}`;
  if (rule.comparison === "Maximum") return `<= ${rule.maximum_value}`;
  if (rule.comparison === "Range") return `${rule.minimum_value} - ${rule.maximum_value}`;
  if (rule.comparison === "Equals") return rule.expected_value || "-";
  return "Required";
}
  ...
  return <main className={styles.page}>
    ...
    <div className={styles.workspace}><section className={styles.sets}><div className={styles.sectionHead}><div><span>Controlled Sets</span><h2>Criteria Library</h2></div><strong>{visibleSets.length}</strong></div><div className={styles.setList}>{visibleSets.map((item) => <button type="button" key={item.id} data-active={item.id === selectedSetId} onClick={() => { const next = (data?.versions ?? []).filter((version) => version.criteria_set_id === item.id).sort((a, b) => b.version_number - a.version_number)[0] ?? null; setSelectedSetId(item.id); setSelectedVersionId(next?.id || ""); setVersionForm(versionFields(next)); }}><strong>{item.name}</strong><span>{item.standard_type} / {item.component_type}</span><small>{item.customer_name || "Company criteria"}</small></button>)}{!visibleSets.length ? <div className={styles.empty}>No criteria sets have been created.</div> : null}</div></section>
      <section className={styles.detail}>{selectedSet && selectedVersion ? <><div className={styles.sectionHead}><div><span>{selectedSet.standard_type} / {selectedSet.component_type}</span><h2>{selectedSet.name}</h2></div><div className={styles.versionActions}><button type="button" onClick={() => editSet(selectedSet)}>Edit Set</button><button type="button" onClick={() => void send({ action: "archive-set", setId: selectedSet.id }, "Criteria set archived.")} disabled={saving}>Archive</button></div></div><div className={styles.detailBody}>
        ...
        <div className={styles.sectionHead}><div><span>{rules.length} Rules</span><h2>Acceptance Rules</h2></div>{editable ? <button className={styles.primary} type="button" onClick={() => setRuleForm({ ...emptyRule, displayOrder: String(rules.length + 1) })}>Add Rule</button> : null}</div>
        <div className={styles.ruleTable}><table><thead><tr><th>Order</th><th>Rule</th><th>Field / Area</th><th>Acceptance</th><th>Result</th><th>Reason</th><th></th></tr></thead><tbody>{rules.map((rule) => <tr key={rule.id}><td>{rule.display_order}</td><td><strong>{rule.rule_name}</strong>{!rule.is_active ? <small> Inactive</small> : null}</td><td>{rule.field_label}<br /><small>{rule.inspection_area}</small></td><td>{criteriaValue(rule)}</td><td>{rule.result_classification}</td><td>{rule.reason}</td><td><div className={styles.ruleActions}>{editable ? <><button type="button" onClick={() => editRule(rule)}>Edit</button><button type="button" onClick={() => { if (window.confirm(`Delete ${rule.rule_name}?`)) void send({ action: "delete-rule", versionId: selectedVersion.id, ruleId: rule.id }, "Rule deleted."); }}>Delete</button></> : null}</div></td></tr>)}{!rules.length ? <tr><td colSpan={7}>No rules are defined for this version.</td></tr> : null}</tbody></table></div>
    ...
    {ruleForm && selectedVersion && selectedSet ? <div className={styles.backdrop}><section className={styles.modal} role="dialog" aria-modal="true"><div className={styles.modalHead}><div><span>Version {selectedVersion.version_number} Draft</span><h2>{ruleForm.ruleId ? "Edit Rule" : "Add Rule"}</h2></div><button type="button" aria-label="Close" onClick={() => setRuleForm(null)}>X</button></div><div className={styles.formGrid}><label><span>Rule Name</span><input value={ruleForm.ruleName} onChange={(event) => setRuleForm({ ...ruleForm, ruleName: event.target.value })} /></label><label><span>Inspection Field</span><select value={ruleForm.fieldKey} onChange={(event) => { const field = fields.find((item) => item.key === event.target.value); setRuleForm({ ...ruleForm, fieldKey: event.target.value, inspectionArea: field?.end || "Joint" }); }}><option value="">Select field</option>{fields.map((field) => <option key={field.key} value={field.key}>{field.group} / {field.label}</option>)}</select></label><label><span>Inspection Area</span><select value={ruleForm.inspectionArea} onChange={(event) => setRuleForm({ ...ruleForm, inspectionArea: event.target.value })}>{["Tube","Box","Pin","Joint"].map((value) => <option key={value}>{value}</option>)}</select></label><label><span>Comparison</span><select value={ruleForm.comparison} onChange={(event) => setRuleForm({ ...ruleForm, comparison: event.target.value })}>{comparisons.map((value) => <option key={value}>{value}</option>)}</select></label>{["Minimum","Range"].includes(ruleForm.comparison) ? <label><span>Minimum Value</span><input type="number" step="any" value={ruleForm.minimumValue} onChange={(event) => setRuleForm({ ...ruleForm, minimumValue: event.target.value })} /></label> : null}{["Maximum","Range"].includes(ruleForm.comparison) ? <label><span>Maximum Value</span><input type="number" step="any" value={ruleForm.maximumValue} onChange={(event) => setRuleForm({ ...ruleForm, maximumValue: event.target.value })} /></label> : null}{ruleForm.comparison === "Equals" ? <label><span>Required Value</span><input value={ruleForm.expectedValue} onChange={(event) => setRuleForm({ ...ruleForm, expectedValue: event.target.value })} /></label> : null}<label><span>Result Classification</span><select value={ruleForm.resultClassification} onChange={(event) => setRuleForm({ ...ruleForm, resultClassification: event.target.value })}>{classifications.map((value) => <option key={value}>{value}</option>)}</select></label><label><span>Display Order</span><input type="number" min="0" step="1" value={ruleForm.displayOrder} onChange={(event) => setRuleForm({ ...ruleForm, displayOrder: event.target.value })} /></label><label className={styles.full}><span>Classification Reason</span><textarea value={ruleForm.reason} onChange={(event) => setRuleForm({ ...ruleForm, reason: event.target.value })} /></label></div><div className={styles.modalActions}><button type="button" onClick={() => setRuleForm(null)}>Cancel</button><button type="button" disabled={saving || !ruleForm.ruleName || !ruleForm.fieldKey || !ruleForm.reason} onClick={() => void send({ action: "save-rule", versionId: selectedVersion.id, ...ruleForm }, ruleForm.ruleId ? "Rule updated." : "Rule added.")}>{saving ? "Saving..." : "Save Rule"}</button></div></section></div> : null}
  </main>;
}
