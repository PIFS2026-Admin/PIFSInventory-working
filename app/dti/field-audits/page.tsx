...
type AuditResponse = { ok?: boolean; metrics?: { audits: number; actionRequired: number; openFindings: number; overdue: number }; checklist?: ChecklistItem[]; audits?: Audit[]; findings?: Finding[]; jobs?: Job[]; people?: Person[]; documents?: JobDocument[]; error?: string };
...
export default function FieldAuditsPage() {
  ...
  const [message, setMessage] = useState("Loading field audits...");
  const [tab, setTab] = useState<"findings" | "audits">("findings");
  ...
  const scorePreview = useMemo(() => {
    ...
    const points = rated.reduce((sum, item) => sum + (ratings[item.item_code] === "C" ? 2 : ratings[item.item_code] === "NI" ? 1 : 0), 0);
    const percent = rated.length ? Math.round((points / (rated.length * 2)) * 10000) / 100 : 0;
  ...
  async function fileAudit() {
    ...
    if (saved) { setActionMessage("Field audit filed and findings created."); setShowNewAudit(false); setTab("findings"); resetAudit(); }
  }
  ...
  return <main className={styles.page}>
    <header className={styles.header}><div className={styles.titleBlock}><Image src="/titan_logo.jpg" alt="TITAN" width={64} height={42} priority /><div><span>DTI</span><h1>Field Audits</h1></div></div><div className={styles.headerActions}><button type="button" onClick={() => goBackOrFallback("/service-lines/dti")}>Back</button><Link href="/dti">DTI Management</Link><Link href="/dti/competency">Competency</Link><button type="button" onClick={() => void loadAudits()}>Refresh</button></div></header>
    ...
    <section className={styles.toolbar}><div className={styles.tabs}><button className={tab === "findings" ? styles.activeTab : ""} type="button" onClick={() => setTab("findings")}>Corrective Actions</button><button className={tab === "audits" ? styles.activeTab : ""} type="button" onClick={() => setTab("audits")}>Audit History</button></div><label><span>Search</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Job, customer, rig, lead, finding..." /></label>{tab === "findings" ? <label><span>Status</span><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option>Open work</option><option>Open</option><option>Action Assigned</option><option>Closed</option><option>All statuses</option></select></label> : <div />}<button className={styles.primaryButton} type="button" onClick={() => { setShowNewAudit(true); setActionMessage(""); }}>New Field Audit</button></section>
    {actionMessage && !selectedFinding ? <div className={styles.actionMessage}>{actionMessage}</div> : null}
    {message ? <section className={styles.message}>{message}</section> : tab === "findings" ? <section className={styles.listSection}><div className={styles.resultCount}>{visibleFindings.length} findings</div><div className={styles.findingList}>{visibleFindings.map((finding) => { const job = jobsById.get(finding.job_id); const overdue = finding.finding_status !== "Closed" && Boolean(finding.due_date) && finding.due_date! < new Date().toISOString().slice(0, 10); return <article key={finding.id} className={finding.severity === "NC" ? styles.ncFinding : styles.niFinding}><div className={styles.findingHeader}><div><span>{finding.finding_number} / {finding.severity}</span><strong>{finding.finding_text}</strong></div><b data-status={finding.finding_status}>{finding.finding_status}</b></div><dl><div><dt>Job</dt><dd><Link href={`/crm/jobs/${encodeURIComponent(finding.job_id)}`}>{job?.title || "Open job"}</Link></dd></div><div><dt>Owner</dt><dd>{finding.owner_name || "Unassigned"}</dd></div><div><dt>Due</dt><dd className={overdue ? styles.overdue : ""}>{formatDate(finding.due_date)}</dd></div></dl>{finding.corrective_action ? <p><strong>Corrective action:</strong> {finding.corrective_action}</p> : null}<div className={styles.itemActions}>{finding.finding_status === "Open" ? <button type="button" onClick={() => openFinding(finding, "assign")}>Assign Action</button> : finding.finding_status === "Action Assigned" ? <><button type="button" onClick={() => openFinding(finding, "assign")}>Edit Action</button><button className={styles.primaryButton} type="button" onClick={() => openFinding(finding, "close")}>Close with Evidence</button></> : <button type="button" onClick={() => openFinding(finding, "view")}>View Closure</button>}</div></article>; })}</div>{!visibleFindings.length ? <div className={styles.empty}>No findings match these filters.</div> : null}</section> : <section className={styles.listSection}><div className={styles.resultCount}>{visibleAudits.length} audits</div><div className={styles.auditList}>{visibleAudits.map((audit) => { const job = jobsById.get(audit.job_id); const open = expandedAudit === audit.id; return <article key={audit.id}><button className={styles.auditSummary} type="button" onClick={() => setExpandedAudit(open ? "" : audit.id)}><div><span>{audit.audit_number} / {formatDate(audit.audit_date)}</span><strong>{job?.title || "Connected job"}</strong><small>{audit.crew_lead_name} / {audit.service_line}</small></div><div><b data-band={audit.status_band}>{audit.status_band}</b><strong>{Number(audit.overall_percent).toFixed(1)}%</strong><span>{open ? "Close" : "Details"}</span></div></button>{open ? <div className={styles.auditDetails}>{audit.items.map((item) => <div key={item.id} data-rating={item.rating}><span>{item.item_code}</span><strong>{item.item_text}{item.is_critical ? " *" : ""}</strong><b>{item.rating}</b>{item.note ? <small>{item.note}</small> : null}</div>)}</div> : null}</article>; })}</div>{!visibleAudits.length ? <div className={styles.empty}>No audits match this search.</div> : null}</section>}

    {showNewAudit ? <div className={styles.modalBackdrop}><section className={`${styles.modal} ${styles.auditModal}`} role="dialog" aria-modal="true"><div className={styles.modalHeader}><div><span>OMS-201</span><h2>New Field Audit</h2></div><button type="button" onClick={() => setShowNewAudit(false)} aria-label="Close">X</button></div><div className={styles.auditSetup}><label><span>Connected Job</span><select value={jobId} onChange={(event) => setJobId(event.target.value)}><option value="">Select job</option>{jobs.map((job) => <option key={job.id} value={job.id}>{job.job_number} / {job.title} / {job.service_line}</option>)}</select></label><label><span>Crew Lead</span><select value={crewLeadId} onChange={(event) => setCrewLeadId(event.target.value)}><option value="">Select crew lead</option>{people.map((person) => <option key={person.id} value={person.id}>{person.full_name}{person.department ? ` / ${person.department}` : ""}</option>)}</select></label><label><span>Audit Date</span><input type="date" value={auditDate} onChange={(event) => setAuditDate(event.target.value)} /></label><label className={styles.fullField}><span>Audit Notes</span><textarea value={notes} onChange={(event) => setNotes(event.target.value)} /></label></div><div className={styles.scoreBar}><div><span>Completed</span><strong>{scorePreview.completed} / {checklist.length}</strong></div><div><span>Score</span><strong>{scorePreview.percent.toFixed(1)}%</strong></div><div><span>Critical NC</span><strong>{scorePreview.critical}</strong></div><div><span>Standing</span><strong data-band={scorePreview.band}>{scorePreview.band}</strong></div></div><div className={styles.checklist}>{sections.map((section) => { const open = openSections.has(section.code); const completed = section.items.filter((item) => ratings[item.item_code]).length; return <section key={section.code}><button className={styles.sectionToggle} type="button" onClick={() => setOpenSections((current) => { const next = new Set(current); if (next.has(section.code)) next.delete(section.code); else next.add(section.code); return next; })}><span>{section.code}</span><strong>{section.title}</strong><small>{completed}/{section.items.length} / {open ? "Close" : "Open"}</small></button>{open ? <div className={styles.checkItems}><div className={styles.sectionFill}><span>Set entire section</span>{(["C","NI","NC","NA"] as const).map((rating) => <button key={rating} type="button" onClick={() => setSectionRating(section.code, rating)}>{rating === "NA" ? "N/A" : rating}</button>)}</div>{section.items.map((item) => <article key={item.item_code}><div className={styles.itemText}><span>{item.item_code}{item.is_critical ? " * Critical" : ""}</span><strong>{item.item_text}</strong><small>{item.reference_text}</small></div><div className={styles.ratingButtons}>{(["C","NI","NC","NA"] as const).map((rating) => <button key={rating} className={ratings[item.item_code] === rating ? styles.selectedRating : ""} data-rating={rating} type="button" onClick={() => setRatings({ ...ratings, [item.item_code]: rating })}>{rating === "NA" ? "N/A" : rating}</button>)}</div><input value={itemNotes[item.item_code] || ""} onChange={(event) => setItemNotes({ ...itemNotes, [item.item_code]: event.target.value })} placeholder="Observation note" /></article>)}</div> : null}</section>; })}</div>{actionMessage ? <div className={styles.actionMessage}>{actionMessage}</div> : null}<div className={styles.modalActions}><button type="button" onClick={() => setShowNewAudit(false)} disabled={saving}>Cancel</button><button className={styles.primaryButton} type="button" onClick={() => void fileAudit()} disabled={saving || scorePreview.completed !== checklist.length}>{saving ? "Filing..." : "File Audit"}</button></div></section></div> : null}
