...
export type DailySummaryConfig = {
  serviceLine: "DTI" | "Tubing";
  tableName: "dti_daily_summaries" | "tubing_daily_summaries";
  summaryPrefix: "DTI" | "TU";
  summaryHref: string;
  moduleHref: string;
  moduleLabel: string;
  readableRoles: Role[];
  editableRoles: Role[];
  showDtiPerformance?: boolean;
};
...
function normalizeRole(role: unknown): Role {
  ...
  if (value === "admin" || value === "employee" || value === "sales" || value === "operator" || value === "dti_superintendent" || value === "dti_inspector" || value === "dti_lead" || value === "level_2_inspector" || value === "service_line_manager" || value === "tubing_lead" || value === "tubing_hand") {
    return value;
  }
  return "customer";
}
...
export default function DailySummaryPage({ config }: { config: DailySummaryConfig }) {
  ...
  const [message, setMessage] = useState(`Loading ${config.serviceLine} daily summaries...`);
  ...
  async function loadPage() {
    ...
    if (!config.readableRoles.includes(role)) {
    ...
    const nextProfile = {
      id: profileData.id,
      fullName: profileData.full_name ?? user.email ?? `${config.serviceLine} User`,
      role,
    };

    setProfile(nextProfile);
    setForm(blankForm(nextProfile.fullName));
    await Promise.all([loadSummaries(), config.showDtiPerformance ? loadPerformanceData() : Promise.resolve()]);
  }
  ...
  async function makeSummaryNumber(dateText: string) {
    const base = `${config.summaryPrefix}-SUM-${ticketDateStamp(dateText)}`;
  ...
  async function uploadInspectionReport(summaryId: string, summaryNumber: string) {
    ...
    const filePath = `${config.serviceLine.toLowerCase()}-daily-summaries/${summaryNumber}/${Date.now()}-${safeFileName(inspectionReportFile.name)}`;
  ...
  async function postSummaryToGroupMe(summaryId: string) {
    ...
    const response = await fetch("/api/dti-summary-groupme", {
      ...
      body: JSON.stringify({ summaryId, serviceLine: config.serviceLine }),
    });
  ...
  function openPrint() {
    ...
    window.open(`${config.summaryHref}/print?id=${form.id}`, "_blank");
  }
    ...
    <main className={`dashboard-shell daily-summary-shell ${summaryStyles.fieldPage}`}>
      <header className="dashboard-header titan-page-header">
        <button className="brand compact brand-home-link" type="button" onClick={() => (window.location.href = "/home")}>
          ...
          <div>
            <div className="brand-title">{config.serviceLine} Daily Summary</div>
        ...
        <div className="dashboard-actions">
          ...
          <button className="button" onClick={() => (window.location.href = config.moduleHref)}>{config.moduleLabel}</button>
      ...
      <section className="customer-welcome">
        <span>Welcome</span>
        <h1>{profile?.fullName ?? `${config.serviceLine} Daily Summary`}</h1>
      ...
      <section className="summary-workspace">
        <aside className="summary-list-panel">
          ...
          {config.showDtiPerformance && <section className="summary-performance-panel">
