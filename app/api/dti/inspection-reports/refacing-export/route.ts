...
import { buildDtiRefacingWorkbook } from "../../../../../lib/dtiRefacingWorkbook";
...
export async function GET(request: Request) {
  try {
    ...
    if (reportResult.error) throw reportResult.error; if (itemsResult.error) throw itemsResult.error;
    if (!reportResult.data) return Response.json({ error: "Inspection report not found." }, { status: 404 });
    const output = await buildDtiRefacingWorkbook(reportResult.data, itemsResult.data ?? []);
    const base = clean(reportResult.data.report_number).replace(/[^a-z0-9_-]+/gi, "-") || "DTI-Refacing-Report";
    return new Response(Uint8Array.from(output).buffer, { headers: { "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Content-Disposition": `attachment; filename="${base}-RFX.xlsx"`, "Cache-Control": "no-store" } });
  } catch (error) {
    console.error("DTI refacing workbook export failed", error);
